...
@echo		T (백업)		규칙을 현재 시간 이름으로 백업 합니다.
...
set choice=
set /p choice=선택 하세요.
echo "%choice%".|findstr /x /i "\"[yndcrbst]\"\." 2>nul>nul&&goto :%choice%||goto :main
...
type %temp%\out.txt | findstr /blv "# 0.0.0. 192.168.0.0/16 224.0.0.0/16 172.30. 192.168. 224.0. 168.126. 210.220. 219.250. 61.41. 1.214. 164.124. 203.248. 180.182. 94.140. 208.67. 1.1. 1.0. 8.8. 9.9. 149.112. 194.242. 185.222. 45.11. 10.0. 172.162" > %temp%\out27777.txt

REM " 20230104 한국 dns 추가 https://minabox.tistory.com/55"
type %temp%\out27777.txt | findstr /blv "# 0.0.0. 219.250. 210.220. 164.124. 203.248. 180.182. 58.143. 59.12. 202.30. 203.240. 149.112. 168.94. 68.94. 208.67." > %temp%\out2.txt
...
:t
REM 규칙을 백업 합니다
REM del /f "%temp%\advfirewall_file.wfw" 2> nul

REM "AI가 만들어 준 부분 입니다 ㅎ"
REM "현재 시간 및 날짜를 가져오기"
...
REM 파일 이름 생성하기
set "filename=advfirewall_file_!year!!month!!day!_!hour!!minute!!second!.wfw"
...
REM 파일 경로와 이름 출력
echo 저장한 파일: !filepath!

echo.
echo [알림: 방어벽 설정]
echo 윈도우키+R  WF.msc
echo.
echo "복구 방법은 netsh advfirewall import "!filepath!" 입니다 wfw 파일 이름은 매번 달라 질수 있으니 직접"
echo.
echo 이렇게 하셔서 가셔서 찾으세요. cd %temp%
echo.
echo "지금 백업한 파일 전체 목록 입니다"
dir %temp%\*.wfw
goto :quit
