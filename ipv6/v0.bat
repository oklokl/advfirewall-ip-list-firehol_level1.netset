...
REM "20221111 이전 버전 7zip 강재 설치 버전"
REM "안타깝게 7zip을 필요 없이 설치 해야 합니다 개발자가 압축파일을 7zip로만 하였네요"
REM "winget install "7zip" --id "7zip.7zip" -s "winget" --accept-package-agreements"
REM "powershell -Command "& {Invoke-WebRequest -Uri "https://lists.ipthreat.net/file/ipthreat-lists/threat/threat-100.txt.gz" -OutFile $env:temp\threat-100.txt.gz}" "
REM "md %temp%\threat"
REM "copy %temp%\threat-100.txt.gz %temp%\threat\threat-100.txt.gz /y"
REM " "%ProgramFiles%\7-Zip\7z.exe" x "%temp%\threat\threat-100.txt.gz" -o"%temp%\threat" "
...
REM "PowerShell -Command "& {$FILE = Get-Content """"%temp%\threat\threat-100.txt"""";$SaveFileDir = """"%temp%\out444.txt"""";foreach ($LINE in $FILE) {foreach($Line in $LINE.Split(' # ')) {Write-Output $Line >> $SaveFileDir;break;}}}" "
REM "20221111 이전 버전 7zip 강재 설치 버전"
...
REM "ipv6 추가버전"
powershell -Command "& {Invoke-WebRequest -Uri "https://lists.ipthreat.net/file/ipthreat-lists/threat/threat-0.txt" -OutFile $env:temp%\threat-0.txt}"
...
PowerShell -Command "& {Get-Content %temp%\threat-0.txt | %%{$_ -replace ' # [ -~]*'} > %temp%\out555.txt}"
...
REM 다른 이름으로 저장
copy %temp%\out.txt "%temp%\out_BAD_ip22.csv" /y

REM 200 줄씩 가져와서 한 줄로 만들어서 화면에 출력.
REM 200 줄씩 ip 등록하기
set /a CountLine=0
set "str="
for /f "delims=" %%a in ('type "%temp%\out_BAD_ip22.csv"') do (
set /a CountLine=!CountLine!+1
set "str=!str!,%%a"
if !CountLine! EQU 200 (
set /a CountLine=0
