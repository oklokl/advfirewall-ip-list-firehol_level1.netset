...
:main
@echo. 
@echo		"해당 iplists.firehol.org 그리고 IPBAN IPThreat 에서 무료로 제공 하는 IP 차단 필터 입니다 불량한 IP를 차단 합니다."
@echo		"기존 방어벽에 나쁜 아이피 차단을 등록 하는 것 뿐입니다. 등록후 윈도우키 + R.. WF.msc 로 살펴 보세요"
...
REM type %temp%\ip2location_country_ru.netset >> %temp%\out.txt
type %temp%\ip2location_country_kp.netset >> %temp%\out.txt
type %temp%\threat\threat-100.txt >> %temp%\out.txt
